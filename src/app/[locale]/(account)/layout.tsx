...
import { unreadCount, unreadInboxCount } from "@/contexts/support/queries";
...
export default async function AccountLayout({ children, params }: { children: ReactNode; params: Promise<{ locale: string }> }) {
  ...
  try {
    const [appts, claims, unread, unreadInbox] = await Promise.all([myAppointments(user.id), myDietClaims(user.id, loc), unreadCount(user.id), unreadInboxCount(user.id)]);
    badges = {
