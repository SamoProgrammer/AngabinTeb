...
- **Pending feedback law (central, no exceptions):** no silent transitions — every submit and every in-flow navigation shows pending state. Server-action submits use `PendingButton` (`@/components/clinical/pending-button`, `useFormStatus` spinner + disable), shadcn admin submits use `PendingAdminButton` (same, shadcn-styled), native GET filter submits use `PendingSubmit` (click-until-unload; only in forms with no `required` fields). In-flow navigations (wizard steps, filters, tabs, pagination, back Links, prominent flow CTAs) use `PendingLink` (`useLinkStatus` spinner veil + `busyLabel={ts("loading")}` from `states.loading`); breadcrumbs and plain cross-page cards stay native `Link` (route `loading.tsx` covers them). Client `onSubmit` handlers keep their own local pending flag. Raw `<button type="submit">` in a server-action form and bare filter/pagination `Link`s are forbidden — grep `type="submit"` and `<Link` when touching a flow.
